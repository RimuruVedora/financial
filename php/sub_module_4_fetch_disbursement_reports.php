...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ticketEntry) {
        try {
            ...
            // 1️⃣ Update ticket_payable (set date_completed)
            $stmt = $conn->prepare("
                ...
                SET Requested_Amount = ?, 
                    ...
                    balance = (? - ?), 
                    status = 'completed',
                    date_completed = NOW()  -- ✅ update completed date
            ...
            echo json_encode(["success" => true, "message" => "Disbursement updated, department budget adjusted, revenue decreased, and completion date set."]);
        } catch (Exception $e) {
            $conn->rollback();
